defmodule BoardTest do
  ...
  test "get all two moves" do
    assert Checkers.Core.Board.all_possible_moves(
      ...
      %Checkers.Core.Move{capture: nil, color: :white, from: {3, 1}, to: {2, 2}},
      %Checkers.Core.Move{capture: nil, color: :white, from: {3, 1}, to: {4, 2}}
    ]
  end
  test "get all two captures" do
    assert Checkers.Core.Board.all_possible_moves(
      ...
      %Checkers.Core.Move{capture: {2, 2}, color: :white, from: {3, 1}, to: {1, 3}},
      %Checkers.Core.Move{capture: {4, 2}, color: :white, from: {3, 1}, to: {5, 3}}
    ]
  end
  test "get only expected color" do
    assert Checkers.Core.Board.all_possible_moves(
      ...
      %Checkers.Core.Move{capture: nil, color: :black, from: {1, 3}, to: {2, 2}},
    ]
  end
end
